...
CGW 是 Cursor 的本机网关客户端：**默认走你已登录的 Cursor 账号**，需要时可**无感切换**到团队号池或自建网关线路。
...
- **双槽分流切号**：在本机 Cursor 账号与团队号池之间切换，切换过程尽量无感，不用反复登录
- **本机优先**：默认使用你电脑上已登录的 Cursor 账号，不依赖远端 Token
- **按需接网关**：只有要用团队计费、号池或远程转发时，才连到自建网关
...
- 网关服务端（`cgw-server`）不在此发布，需自行部署
- 版本清单与本机服务二进制存放在仓库加密目录中，由程序自动读取，无需手动关注
